## Практичне заняття № 5. Вирішення задач маршрутизації евристичними методами

**Тема:** Евристичні алгоритми (Жадібний пошук, Монте-Карло, імітація відпалу)  
...
де $F_a(n^2)$ відображає часову складність $\mathcal{O}(n^2)$, $M_{code}$ позначає обсяг програмного коду, $S_d$ відповідає розміру пам'яті під матрицю $D$ ($S_d = n^2 \cdot \text{sizeof}(double)$), $S_r$ описує витрати стека, а $c_1, c_2, c_3, c_4$ є ваговими коефіцієнтами платформи.
...
2.  **Завдання варіанта:** Таблиця з вхідними параметрами вашого варіанта (Назва системи КФС, кількість точок $n$, початковий вузол $P_1$, діапазон координат, кількість вибірок Монте-Карло).
