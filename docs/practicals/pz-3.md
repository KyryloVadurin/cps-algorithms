## Практичне заняття № 3. Проєктивання ієрархії тем MQTT для КФС

**Тема:** Протоколи обміну даними та маршрутизація телеметрії в IIoT  
...
У наведеній формулі змінна $L$ позначає підсумкову обчислену довжину корисного навантаження у байтах, параметр $n$ визначає кількість байтів, використаних для кодування довжини (від $1$ до $4$), змінна $B_i$ відповідає значенню $i$-го байта поля довжини, а побітова операція $\& \ 127$ виконує побітове «І» з числом 127 для обнулення старшого біта продовження.
...
2.  **Завдання варіанта:** Таблиця з вхідними параметрами вашого варіанта (Назва підприємства, цехи, пристрої, параметри, розмір кадру, маска підписки, QoS).
